...
class DatabaseManager:
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT users.user_name, COUNT(winners.prize_id) AS count_prize FROM winners INNER JOIN users ON users.user_id = winners.user_id GROUP BY winners.user_id ORDER BY count_prize LIMIT 10')
        return cur.fetchall()
